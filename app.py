...
geology = my_form.number_input("Select the geology type of that area",min_value=1, max_value=7)
...
def preprocessing_data(data, one_hot_enc, scaler):
    ...
    return final_data
# function to clean and tranform the input   data1,
# def preprocessing_data(data, scaler):

    # Convert the following numerical labels from integer to float
    # float_array= data["elevation"].values.astype(
    #     float
    # )


    # One Hot Encoding conversion
    # data = one_hot_encoding.transform(data)
...
if submit:
    ...
    if output == 1:
        st.write(
            "There is a probability of landslide to occur of {} 😊".format(
                probability
            )
        )
    elif output == 0:
        st.write(
            "There is no probability of landslide to occur of {} 😔".format(
                probability
            )
        )

st.write("Developed with ❤️ by [Group1](%s)")
